...
// ─────────────────────────────────────────────────────────────────────────────
//  Ace Technologies – Selenium Test Suite  ·  Excel Report Generator
// ─────────────────────────────────────────────────────────────────────────────
  ...
  rows.forEach((row, ri) => {
    headers.forEach((col, ci) => {
      ...
      if (col === 'Status') {
        ...
        style.fill = { type: 'pattern', patternType: 'solid',
      ...
      if (ri % 2 === 0) {
        style.fill = style.fill || {
          type: 'pattern', patternType: 'solid', fgColor: { rgb: 'F7F9FC' }
        };
      }
      ws[cellAddr].s = style;
    });
 ...
 *
 * @param {object[]} allResults  Array of { suite, id, title, area, route,
 *                               priority, expected, status, actual,
 *                               durationMs, timestamp, evidence } objects.
 * @param {object[]} allCases    Full case manifest for coverage matrix.
 * @param {Date}     startedAt   When the run started.
 */
function generateReport(allResults, allCases, startedAt) {
  ensureDirs();
  const now      = new Date();
  const filename = `Ace_Technologies_TestReport_${startedAt.toISOString().replace(/[:.]/g, '-')}.xlsx`;
  ...
  const dashboardAoa = [
    ['ACE TECHNOLOGIES – WEB APPLICATION TEST REPORT'],
    [],
    ...
    ['Headless Mode',    CONFIG.browser.headless ? 'Yes' : 'No'],
    [],
  ...
  for (const suite of suites) {
    ...
    if (rows.length) {
      // Trim to 31-char Excel sheet name limit
  ...
  const perfRows = allResults
    .filter((r) => r.suite === 'Performance Testing')
    .map((r) => ({
      'Test ID':       r.id,
      'Test Scenario': r.title,
      'Route':         r.route,
  ...
  if (perfRows.length) {
    appendStyledSheet(wb, 'Performance Summary', perfRows,
      [10, 40, 28, 10, 14, 60, 25]);
  }
